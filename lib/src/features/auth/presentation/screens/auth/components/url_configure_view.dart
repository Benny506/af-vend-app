...
class _UrlConfigureViewState extends State<UrlConfigureView> {
  ...
  final supabaseUrlCtrl = TextEditingController();
  final supabaseAnonKeyCtrl = TextEditingController();
  final supabaseFormKey = GlobalKey<FormState>();
  ...
  MedusaApiVersion medusaApiVersion = MedusaApiVersion.v1;
  ...
  void _loadInit() async {
    ...
    medusaApiVersion = authPreferenceService.authPreference.medusaApiVersion;
    if (authType != AuthenticationType.jwt || medusaApiVersion != MedusaApiVersion.v1) {
      advancedOption = true;
    }
    ...
    if (authType == AuthenticationType.supabase) {
      supabaseUrlCtrl.text =
          await getIt<FlutterSecureStorage>().read(key: AppConstants.supabaseUrlKey) ?? '';
      supabaseAnonKeyCtrl.text =
          await getIt<FlutterSecureStorage>().read(key: AppConstants.supabaseAnonKey) ?? '';
    }
  ...
  Future<void> _save({bool skipValidation = false}) async {
    ...
    if (textCtrl.text == savedBaseUrl &&
        ...
        authPreferenceService.authPreference.medusaApiVersion == medusaApiVersion &&
  ...
  Future<bool> _advancedOptionSettings() async {
    ...
    if (authType == AuthenticationType.supabase) {
      if (!supabaseFormKey.currentState!.validate()) {
        return false;
      }
      await getIt<FlutterSecureStorage>()
          .write(key: AppConstants.supabaseUrlKey, value: supabaseUrlCtrl.text);
      await getIt<FlutterSecureStorage>()
          .write(key: AppConstants.supabaseAnonKey, value: supabaseAnonKeyCtrl.text);
      authPreferenceService.setIsAuthenticated(true);
      try {
        Supabase.instance.client = SupabaseClient(supabaseUrlCtrl.text, supabaseAnonKeyCtrl.text);
      } catch (_) {}
    }

    try {
      await authPreferenceService.updateAuthPreference(authPreferenceService
          .authPreference
          .copyWith(authType: authType, medusaApiVersion: medusaApiVersion));
      ...
      switch (authType) {
        ...
        case AuthenticationType.jwt:
          await getIt<FlutterSecureStorage>().delete(key: AppConstants.tokenKey);
          await getIt<FlutterSecureStorage>().delete(key: AppConstants.cookieKey);
          await getIt<FlutterSecureStorage>().delete(key: AppConstants.supabaseTokenKey);
          break;
  ...
  Widget build(BuildContext context) {
    ...
    String infoText = switch (authType) {
      ...
      AuthenticationType.supabase => 'Use Supabase bypass to send authenticated requests.',
            ...
            children: [
              ...
              if (advancedOption)
                Column(
                  ...
                  children: [
                    const Divider(),
                    Text('Medusa Version', style: context.bodyMedium),
                    const Gap(12.0),
                    SizedBox(
                      width: double.maxFinite,
                      child: SegmentedButton<MedusaApiVersion>(
                          showSelectedIcon: false,
                          onSelectionChanged: (value) =>
                              setState(() => medusaApiVersion = value.first),
                          segments: MedusaApiVersion.values
                              .map((e) => ButtonSegment<MedusaApiVersion>(
                              value: e, label: Text(e.toString())))
                              .toList(),
                          selected: {medusaApiVersion}),
                    ),
